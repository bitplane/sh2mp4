...
class TerminalManager:
    ...
    def __init__(self, display_name: str, theme: Theme, font_name: str, font_size: int, 
                 cols: int, lines: int, width: int, height: int):
...
"""
    ...
    async def _start_xterm(self, script_path: Path) -> None:
        ...
        cmd = [
            ...
            "-geometry", f"{self.cols}x{self.lines}",
            "-T", "sh2mp4_recording",
            "+sb",  # No scrollbar
            "-b", "0",  # No border
    ...
    async def _configure_window(self) -> None:
        ...
        await asyncio.create_subprocess_exec(
            "wmctrl", "-ir", self.window_id, "-e", f"0,0,0,{self.width},{self.height}",
            env=env,
            stdout=asyncio.subprocess.DEVNULL,
            stderr=asyncio.subprocess.DEVNULL
        )
        ...
        # Remove decorations
        await asyncio.create_subprocess_exec(
            "xprop", "-id", self.window_id, "-f", "_MOTIF_WM_HINTS", "32c",
        ...
        await asyncio.create_subprocess_exec(
            "wmctrl", "-i", "-r", self.window_id, "-b", "add,undecorated",
            env=env,
            stdout=asyncio.subprocess.DEVNULL,
            stderr=asyncio.subprocess.DEVNULL
        )
        ...
        # Make fullscreen
        await asyncio.create_subprocess_exec(
            "wmctrl", "-i", "-r", self.window_id, "-b", "add,fullscreen",
            env=env,
            stdout=asyncio.subprocess.DEVNULL,
            stderr=asyncio.subprocess.DEVNULL
        )
    ...
    async def is_running(self) -> bool:
        ...
        return (self.xterm_process is not None and 
                self.xterm_process.returncode is None)
